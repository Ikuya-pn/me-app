...
const storeWork = () => {
    if (confirm('シフトを登録しますか？(確定後にシフトの修正はできません。)')) {
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <form @submit.prevent="handleSubmit" class="w-2/3 md:w-1/3 mx-auto py-8">
                        ...
                        <h2 v-if="form.is_locked === 1 && existFixWork === null" class="font-semibold text-sm text-red-400 leading-tight">{{ day.format('M') }}月{{ props.date }}日のシフトは以下の内容で確定しています。修正がある場合は修正内容を入力してください。</h2>
                        ...
                        <div class="w-full mt-4">
                            <label v-if="form.is_locked === false" for="customer_id" class="text-gray-600 text-sm">勤務先のお客様</label><br>
                            <label v-if="form.is_locked === 1 && existFixWork === null" for="customer_id" class="text-gray-600 text-sm">勤務先のお客様</label><br>
